...
from citation_extractor.io.iob import (
    count_tokens,
    file_to_instances,
    filter_IOB,
    read_iob_files,
...
def extract_entity_mentions(text, citation_extractor, postaggers, norm=False):
    ...
    try:
        ...
        logger.debug(
            'Exception while tagging {} with lang={}'.format(text, lang)
        )
        return []

    # convert to a list of lists; keep just token and PoS tag, discard lemma
    iob_data = [
        [token[:2] for token in sentence] for sentence in [postagged_string]
    ]

    # put the PoS tags into a separate nested list
    postags = [
        [("z_POS", token[1]) for token in sentence]
    ...
    output = [
        [
            (res[n]["token"].decode('utf-8'), postags[i][n][1], res[n]["label"])
    ...
    authors = [(AUTHOR_TYPE, a) for a in filter_IOB(output, "AAUTHOR")]
    works = [(WORK_TYPE, w) for w in filter_IOB(output, "AWORK")]
    ...
    if norm:
        mentions_norm = [
            (
                m_type_m_surface[0],
                StringUtils.normalize(m_type_m_surface[1], lang=lang),
            )
            for m_type_m_surface in mentions
        ]
...
def recover_segmentation_errors(text, abbreviation_list, verbose=False):
    ...
    if verbose:
        print("Input text has %i lines" % len(text_lines), file=sys.stderr)
    for line in text_lines:
        ...
        if len(tokens) == 1:
            ...
            if not is_abbreviation(tokens[0], abbreviation_list):
                ...
                if verbose:
                    print("%s is an abbreviation" % tokens[0], file=sys.stderr)
        else:
            output += tokens
            try:
                ...
                if not is_abbreviation(last_token, abbreviation_list):
                    ...
                    if verbose:
                        print(
                            "%s is an abbreviation" % last_token,
                            file=sys.stderr,
                        )
            except Exception as e:
                pass
    ...
    if verbose:
        print(
            "Output text has %i lines" % len(output_text.split('\n')),
            file=sys.stderr,
        )
        print(
            "%i line were breaks recovered"
            % (len(text_lines) - len(output_text.split('\n'))),
            file=sys.stderr,
        )
    return output_text
...
def get_taggers(
    treetagger_dir='/Applications/treetagger/cmd/', abbrev_file=None
):
    """
    ...
    logger.info(
        "Env variable $TREETAGGER_HOME == %s" % os.environ["TREETAGGER_HOME"]
    )
    lang_codes = {
        'en': ('english', 'utf-8'),
        'it': ('italian', 'utf-8'),
    ...
    for lang in list(lang_codes.keys()):
        try:
            taggers[lang] = TreeTagger(
                language=lang_codes[lang][0],
            ...
            logger.error(
                "initialising Treetagger for language %s raised error: \"%s\""
                % (lang_codes[lang][0], e)
            )
            raise e
    return taggers
...
def detect_language(text, return_probability=False):
    ...
    try:
        ...
        logger.debug(
            ...
            % (language, classification_probability)
        )
...
def compact_abbreviations(abbreviation_dir):
    ...
    for fn in files:
        ...
        print("getting abbreviations from %s" % fn, file=sys.stderr)
...
def split_sentences(filename, outfilename=None):
    ...
    try:
        sent_tok = create_instance_tokenizer(
            train_dirs=[
                ("/Users/rromanello/Documents/APh_Corpus/goldset/txt/", '.txt'),
            ]
        )
        ...
        print(
            "%i sentence breaks were recovered"
            % (len(sentences) - len(new_sentences)),
            file=sys.stderr,
        )
    except Exception as e:
        raise e
...
def preproc_document(
    ...
    try:
        ...
        if split_sentences:
            ...
            text = recover_segmentation_errors(
                intermediate_text, abbreviations, verbose=False
            )
        else:
            logger.info("Document %s: skipping sentence splitting" % doc_id)
        sentences = text.split('\n')
        logger.info(
            "Document \"%s\" has %i sentences" % (doc_id, len(sentences))
        )
        ...
        tokenised_text = [
            [token for token in line] for line in tagged_sentences
        ]
...
def validate_configuration(configuration_parameters, task="all"):
    ...
    if task == "all":
        assert (
            is_valid_configuration_ner(configuration_parameters)
...
def find_input_documents(task, processing_dirs, doc_id):
    ...
    logger.info("There are {} docs to process".format(len(docs_to_process)))
    logger.info(
        'Following documents will be processed: {}'.format(docs_to_process)
    )
...
def do_relex(doc_id, input_dir, output_dir, prefix=None):
    ...
    logger.info(
        'Document {} contains {} entities.'.format(doc_id, len(doc['entities']))
    )
...
def do_ned(doc_id, input_dir, output_dir, matcher):
    ...
    for entity_id in doc['entities']:
        ...
        if e_type == 'REFSCOPE':
            ...
            cleaned_scope = re.sub(cleaning_regex, "", surface).replace(
                "–", "-"
            )
            ...
            try:
                ...
                if result.urn != 'urn:cts:GreekLatinLit:NIL':
                    work_uri = matcher._kb.get_resource_by_urn(
                        result.urn
                    ).subject
                    entity['work_uri'] = work_uri
            except Exception as e:
                logger.error(
                    "There was a problem disambiguating {}".format(
                        surface.encode('utf-8')
                    )
                )
            ...
            "Document {}: total entities {}; disambiguated: {}".format(
                ...
                len(
                    [
                        ...
                        if (
                            'work_uri' in entities[ent_id]
                            or 'author_uri' in entities[ent_id]
                        )
                    ]
                ),
            )
        )
        logger.info("Document {} written to {}".format(doc_id, outp_file_path))
    return
...
def main():
    ...
    if args['init']:
        ...
        if args['preproc']:
            ...
            try:
                abbreviations = (
                    codecs.open(cfg_abbrevations_file).read().split('\n')
                )
            except Exception as e:
            ...
            if cfg_storage is not None and os.path.exists(pkl_extractor_path):
                ...
                logger.info(
                    "Extractor loaded from pickle {}".format(pkl_extractor_path)
                )
            ...
            for doc_id in docs_to_process:
                do_relex(
                    doc_id, input_dir=dirs['json'], output_dir=dirs['json'],
                )
        elif args['ned']:

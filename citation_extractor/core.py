...
class ScikitClassifierAdapter:
    ...
    def __init__(
        ...
        fe = FeatureExtractor()
        logger.info(
            "Initialising SklearnClassifier with model %s" % scikit_classifier
        )
        ...
        if isinstance(scikit_classifier, CRF):
            ...
            for n, instance in enumerate(iob_data):
                ...
                feature_sets.append(
                    self.apply_feature_template(sentence_feats, out_label=True)
                )

            X_train = [
                [token[0] for token in instance] for instance in feature_sets
            ]
            y_train = [
                [token[1] for token in instance] for instance in feature_sets
            ]
            ...
            self.classifier.train(
                self.apply_feature_template(feature_sets, out_label=True)
            )
            return
    ...
    def apply_feature_template(self, feature_sets, out_label=False):
        ...
        if out_label:
            unlabelled_feature_sets = [
                [f[0][key] for key in sorted(f[0])] for f in feature_sets
            ]
        else:
            unlabelled_feature_sets = [
                [f[key] for key in sorted(f)] for f in feature_sets
            ]
        ...
        for n, fs in enumerate(unlabelled_feature_sets):
            ...
            for template, replacements in self.compiled_templates:
                ...
                values = [
                    get_value(unlabelled_feature_sets, n + r[0], r[1])
...
class citation_extractor:
    ...
    def __init__(self, options, classifier=None, labelled_feature_sets=None):
        ...
        if options.DATA_FILE != "":
            ...
            chain_IOB_files(
                options.DATA_DIRS, "%sall_in_one.iob" % options.TEMP_DIR, ".txt"
            )
            allinone_iob_file = "%sall_in_one.iob" % options.TEMP_DIR
        # initialise the classifier
        if classifier is None:
            self.classifier = CRFPP_Classifier(
                allinone_iob_file,
                "%s%s" % (options.CRFPP_TEMPLATE_DIR, options.CRFPP_TEMPLATE),
                options.TEMP_DIR,
            )
        else:
            self.classifier = ScikitClassifierAdapter(
                classifier,
                allinone_iob_file,
                "%s%s" % (options.CRFPP_TEMPLATE_DIR, options.CRFPP_TEMPLATE),
    ...
    def extract(self, instances, legacy_features=None):
        ...
        for n, instance in enumerate(instances):
            if legacy_features is not None:
                feat_sets = self.fe.get_features(
                    instances[n], [], False, legacy_features[n]
                )
            else:
...
class FeatureExtractor:
    ...
    def __init__(self):
        ...
        self.feat_labels[
            self.UNPAIRED_ROUND_BRACKETS
        ] = "UNPAIRED_ROUND_BRACKETS"
        self.feat_labels[self.PAIRED_SQUARE_BRACKETS] = "PAIRED_SQUARE_BRACKETS"
        self.feat_labels[
            self.UNPAIRED_SQUARE_BRACKETS
        ] = "UNPAIRED_SQUARE_BRACKETS"
        # case
        ...
        self.feat_labels[
            self.DOT_SEPARATED_PLUS_RANGE
        ] = "DOT_SEPARATED_PLUS_RANGE"
    ...
    def init_dictionaries(self):
        ...
        try:
            ...
            raw_data = file.read()
            file.close()
            self.works_dict = LookupDictionary(raw_data)
        except Exception as e:
            raise e
        ...
        try:
            ...
            raw_data = file.read()
            file.close()
            self.authors_dict = LookupDictionary(raw_data)
        except Exception as e:
            raise e
        return
    ...
    def extract_number_feature(self, check_str):
        ...
        * is an ordinale number (Roman)?
        ...
        res = self.OTHERS
        naked = re.sub(
            '[%s]' % re.escape(string.punctuation), '', check_str
        ).lower()
        ...
        for n, char in enumerate(check_str):
            if char.isalnum():
                if char.isalpha():
                    ...
                    if (
                        n + 1 <= len(check_str) - 1
                        and check_str[n + 1].isdigit()
                    ):
                        pass
                    else:
                        result.append('0')
            else:
                if n + 1 <= len(check_str) - 1 and (
                    check_str[n + 1].isalnum() is False
                ):
                    pass
                else:
                    result.append('-')
    ...
    def extract_features(self, inp):
        feature_set = []
        feat_funcs = [
            self.extract_punctuation_feature,
    ...
    def get_features(
        self, instance, labels=[], outp_label=True, legacy_features=None
    ):
        """
        Args:
            instance:
        ...
        for n, x in enumerate(res):
            ...
            for key in keys:
                if type(x[key]) is type(12):
                    x[key] = self.feat_labels[
                        x[key]
                    ]  # get the string label corresponding to a given int value
    ...
    def prepare_for_training(self, file_name):
        ...
        if len(all_postags) > 0:
            all_postags = [
                [("z_POS", token) for token in instance]
                for instance in all_postags
            ]
            res2 = [
                self.get_features(
                    r, all_labels[n], legacy_features=all_postags[n]
                )
                for n, r in enumerate(all_tokens)
            ]
        else:
            res2 = [
                self.get_features(r, all_labels[n])
                for n, r in enumerate(all_tokens)
            ]
